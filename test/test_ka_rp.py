...
def test_exploit_handler_executor_pert():
    ...
    assert list(bb.get_attr('abstract_lvls')['level 3']['old'].keys()) == ['core_1']
...
def test_exploit_handler_trigger_publish():
    ...
    rp.set_attr(analyzed_design = {'core 1': 'Analyzed'})
    bb.update_abstract_lvl(1, 'core 1', {'pareto type' : 'pareto', 'fitness function' : 1.0})
    bb.publish_trigger()
    time.sleep(0.075)
    bb.controller()
    assert bb.get_attr('_kaar') == {1: {'ka_rp': 0}, 2: {'ka_rp':5.00001}, 3: {'ka_rp': 0}}
    assert bb.get_attr('_ka_to_execute') == (None, 0)
